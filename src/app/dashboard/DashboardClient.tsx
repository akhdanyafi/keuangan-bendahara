...
import { formatRupiah, STATUS_LABEL, STATUS_COLOR } from '@/types'
import {
...
const KATEGORI_ICON: Record<string, React.ReactNode> = {
  'ATK (Alat Tulis Kantor)': <FileText size={16} />,
  'Sarana Prasarana': <ShoppingBag size={16} />,
  'Kegiatan Sekolah': <Award size={16} />,
  'Listrik & Utilitas': <Zap size={16} />,
  'Lainnya': <Package size={16} />,
}

const KATEGORI_COLOR: Record<string, string> = {
  'ATK (Alat Tulis Kantor)': 'bg-blue-100 text-blue-600',
  'Sarana Prasarana': 'bg-amber-100 text-amber-600',
  'Kegiatan Sekolah': 'bg-purple-100 text-purple-600',
  'Listrik & Utilitas': 'bg-green-100 text-green-600',
          ...
          <li key={a.id} className="flex items-center gap-3 py-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-slate-400">
                {a.guru_nama} · {new Date(a.tanggal_pengajuan).toLocaleDateString('id-ID', { day: '2-digit', month: 'short', year: 'numeric' })}
              </p>
            </div>
...
function GuruSection({
  title, icon, iconBg, items, emptyText, linkHref,
}: {
  title: string; icon: React.ReactNode; iconBg: string
  items: GuruPengajuan[]; emptyText: string; linkHref: string
}) {
  return (
    <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
      <div className={`px-5 py-4 flex items-center justify-between border-b border-slate-100`}>
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400">{items.length} pengajuan</p>
          </div>
        </div>
        <a href={linkHref} className="text-xs text-blue-600 hover:underline font-medium">Lihat Semua →</a>
      </div>
      ...
      {items.length === 0 ? (
        ...
        <ul className="divide-y divide-slate-50">
          {items.map((p) => (
            <a key={p.id} href={`/pengajuan`} className="flex items-center gap-4 px-5 py-3.5 hover:bg-slate-50/80 transition-colors block">
              <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${KATEGORI_COLOR[p.kategori_nama] || 'bg-slate-100 text-slate-600'}`}>
                {KATEGORI_ICON[p.kategori_nama] || <HelpCircle size={14} />}
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium text-slate-700 truncate">{p.nama_barang}</p>
                <p className="text-xs text-slate-400">
                  {p.kategori_nama.split('(')[0].trim()} · {new Date(p.tanggal_pengajuan).toLocaleDateString('id-ID', { day: '2-digit', month: 'short', year: 'numeric' })}
                  {p.approved_by_nama && <span className="text-green-600"> · Disetujui oleh {p.approved_by_nama}</span>}
                </p>
              </div>
              <div className="text-right shrink-0">
                <p className="text-sm font-semibold text-slate-700">{formatRupiah(p.estimasi_harga)}</p>
                <span className={`inline-block mt-0.5 text-xs font-medium px-2 py-0.5 rounded-full ${STATUS_COLOR[p.status]}`}>
...
function GuruDashboard({ guruData }: { guruData: { pendingApproval: GuruPengajuan[]; menungguNota: GuruPengajuan[]; aktivitas: GuruPengajuan[] } }) {
  const sections = [
    {
      title: 'Menunggu Approval',
      sub: `${guruData.pendingApproval.length} pengajuan`,
      icon: <Clock size={16} className="text-orange-500" />,
      iconBg: 'bg-orange-50',
      items: guruData.pendingApproval,
      emptyText: 'Tidak ada yang menunggu approval',
      href: '/pengajuan',
    },
    {
      title: 'Menunggu Nota',
      sub: `${guruData.menungguNota.length} pengajuan`,
      icon: <AlertTriangle size={16} className="text-amber-500" />,
      iconBg: 'bg-amber-50',
      items: guruData.menungguNota,
      emptyText: 'Tidak ada yang perlu upload nota',
      href: '/pengajuan',
    },
    {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 h-full">
      {sections.map((s) => (
        <div key={s.title} className="bg-white rounded-xl border border-slate-200 flex flex-col overflow-hidden">
          {/* Card header */}
          <div className="px-4 py-4 border-b border-slate-100 flex items-center justify-between shrink-0">
            <div className="flex items-center gap-2.5">
              <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${s.iconBg}`}>{s.icon}</div>
              <div>
                <h3 className="text-sm font-semibold text-slate-800">{s.title}</h3>
                <p className="text-xs text-slate-400">{s.sub}</p>
              </div>
            </div>
            <a href={s.href} className="text-xs text-blue-600 hover:underline font-medium shrink-0">Semua →</a>
          </div>
          ...
          <div className="flex-1 overflow-y-auto">
            {s.items.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-32 text-slate-300">
                ...
                <p className="text-xs text-slate-400 text-center px-4">{s.emptyText}</p>
              </div>
            ) : (
              <ul className="divide-y divide-slate-50">
                {s.items.map((p) => (
                  <a key={p.id} href={s.href}
...
/* ── Full Dashboard (Kepala Sekolah & Bendahara) ── */
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-5">
          ...
          <ul className="space-y-4">
            {data.budgetKategori.map((b: { nama: string; budget: number; terpakai: number }) => {
              ...
              return (
                <li key={b.nama}>
                  <div className="flex items-center justify-between text-xs mb-1.5">
                    <span className="text-slate-600 font-medium truncate max-w-[120px]">{b.nama.split('(')[0].trim()}</span>
                  ...
                  <div className="flex justify-between mt-1">
                    <span className="text-xs text-slate-400">{formatRupiah(Number(b.terpakai))}</span>
...
export default function DashboardClient({ session }: { session: SessionPayload }) {
  ...
  const [guruData, setGuruData] = useState<{ pendingApproval: GuruPengajuan[]; menungguNota: GuruPengajuan[]; aktivitas: GuruPengajuan[] } | null>(null)
  ...
  return (
    <DashboardLayout title="Dashboard" role={session.role} nama={session.nama}>
      {session.role === 'guru' && guruData
        ? <GuruDashboard guruData={guruData} />
